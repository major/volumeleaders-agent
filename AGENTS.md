...
- Trade and cluster `core` presets use compact derived fields. `CalendarEvent` contains true upstream calendar markers (`EOM`, `EOQ`, `EOY`, `OPEX`, `VOLEX`); array output returns `null` when none are true and object output omits it. `AuctionTrade` derives from upstream `OpeningTrade` and `ClosingTrade` `0`/`1` flags; array output returns `"open"`, `"close"`, or `null`, and object output omits it when neither flag is true.
