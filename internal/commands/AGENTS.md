...
- Treat broken stdout/stderr discipline as P1. Command data belongs on stdout as compact JSON by default or CSV/TSV where supported, while diagnostics and errors belong on stderr through `slog`.
...
- Verify command behavior matches README and `skills/volumeleaders-agent/*.md`. If commands, flags, models, defaults, or output formats change, require matching skill documentation updates.
